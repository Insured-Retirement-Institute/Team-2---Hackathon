...
export function getMockPolicyData(alert: RenewalAlert): PolicyData {
  return {
    ...
    features: {
      ...
      features: [
        { name: "Multi-Year Rate Guarantee", description: "Fixed interest rate guaranteed for the full term with no market volatility risk.", included: true, type: "feature" },
        { name: "Principal Protection", description: "100% principal protection from market downturns.", included: true, type: "feature" },
        { name: "Guaranteed Growth", description: "Interest compounds daily and is credited annually. Minimum guaranteed rate of 1.50%.", included: true, type: "feature" },
        { name: "1035 Exchange Eligible", description: "Can be exchanged tax-free for another qualified annuity under IRC Section 1035.", included: true, type: "feature" },
      ],
      benefits: [
        { name: "Tax-Deferred Growth", description: "Interest earnings grow tax-deferred until withdrawal.", included: true, type: "benefit" },
        { name: "Death Benefit", description: "Beneficiaries receive full account value without reduction.", included: true, type: "benefit" },
        { name: "Annual Free Withdrawal", description: "10% of account value can be withdrawn each year without penalty.", included: true, type: "benefit" },
        { name: "Nursing Home Waiver", description: "Full surrender charge waiver if confined to nursing facility for 90+ consecutive days.", included: true, type: "benefit" },
      ],
      riders: [
        { name: "Enhanced Death Benefit Rider", description: "Provides an additional 10% bonus to death benefit proceeds.", included: false, type: "rider" },
        { name: "Income Guarantee Rider", description: "Guarantees minimum income percentage regardless of account value.", included: false, type: "rider" },
        { name: "Long-Term Care Acceleration", description: "Allows accelerated access to death benefit if qualified for long-term care.", included: false, type: "rider" },
      ],
      limitations: [
        { name: "Surrender Charge Period", description: "7 of 10-year surrender period remaining. Early withdrawal beyond free amount subject to 2.5% penalty.", included: true, type: "limitation" },
        { name: "Market Value Adjustment (MVA)", description: "Surrenders during guarantee period subject to MVA. Current penalty is -3.8%.", included: true, type: "limitation" },
        { name: "Liquidity Restrictions", description: "Limited access to funds beyond 10% annual free withdrawal.", included: true, type: "limitation" },
        { name: "Rate Renewal Risk", description: "At end of guarantee period, renewal rate may be significantly lower.", included: true, type: "limitation" },
        { name: "No Market Upside Participation", description: "Fixed rate means no ability to benefit from rising interest rates or equity market gains.", included: true, type: "limitation" },
      ],
    },
  };
}
...
export const mockSuitabilityData: SuitabilityData = {
  clientObjectives: "Principal protection with moderate growth, estate planning for beneficiaries",
  riskTolerance: "conservative",
  timeHorizon: "long",
  liquidityNeeds: "Minimal: Rarely need access",
  taxConsiderations: "Tax efficiency: High priority - Critical priority. Deferred Income: Distributions in 5+ years",
  guaranteedIncome: "preferred",
  rateExpectations: "Rate expectations: Above Average - Better than market rates",
...
export const mockDisclosureItems: DisclosureItem[] = [
  { id: "d1", title: "Product Disclosure Review", description: "I have reviewed all product disclosures with the client", documentName: "Form ADV Part 2A - Client Brochure", required: true },
  { id: "d2", title: "Surrender Period & Withdrawals", description: "Client understands the surrender period and withdrawal restrictions", documentName: "Annuity Surrender Schedule Disclosure", required: true },
  { id: "d3", title: "Replacement Transaction Notice", description: "Client acknowledges this is a replacement transaction subject to regulatory review", documentName: "NAIC Form 1035 Exchange Notice", required: true },
  { id: "d4", title: "Suitability Documentation", description: "I have documented the complete suitability rationale in the client file", documentName: "Client Suitability Worksheet (Internal)", required: true },
  { id: "d5", title: "Comparison Disclosure Form", description: "Client has been provided with and reviewed the required comparison disclosure form", documentName: "Product Comparison & Cost Disclosure", required: true },
];

export function getMockTransactionOptions(alert: RenewalAlert, comparisonData: ComparisonData): TransactionOption[] {
  const options: TransactionOption[] = [
    {
      ...
      requirements: [],
      warnings: [],
      pros: ["No new paperwork or underwriting", "Maintains existing carrier relationship", "Surrender period already expired or minimal remaining"],
      cons: [`Significant rate drop to guaranteed minimum ${alert.renewalRate}`, `Missing opportunity to capture higher market rates (${comparisonData.alternatives[0]?.rate || "4.2%"} available)`, "Lower returns for client over policy term"],
      isAvailable: true,
    },
  ];
  ...
  comparisonData.alternatives.forEach((alt) => {
    ...
    pros.push(`Rate improvement: ${(newRate - currentRate).toFixed(1)}% increase (${alert.renewalRate} → ${alt.rate})`);
    if (alt.premiumBonus) pros.push(`${alt.premiumBonus} premium bonus on entire account value`);
    pros.push("Enhanced features including death benefit protection", `Strong carrier rating (${alt.carrier})`);
    ...
    options.push({
      ...
      description: "",
      requirements: [
        alt.licensingApproved ? `Licensed with ${alt.carrier} - Verified ✓` : `Licensing required with ${alt.carrier}`,
        "1035 exchange documentation",
        "State replacement forms",
        `New surrender period applies (${alt.surrenderPeriod} years)`,
      ],
      warnings: [`New surrender charges apply for ${alt.surrenderPeriod} years`],
      pros,
      cons: [`New ${alt.surrenderPeriod}-year surrender period begins`, "Requires 1035 exchange paperwork", "State replacement forms and review period", "Client must acknowledge new terms and conditions"],
...
export const mockAuditLog: AuditLogEntry[] = [
  { timestamp: new Date().toLocaleString(), user: "advisor@firm.com", action: "Alert viewed", details: "Policy opened for review" },
  { timestamp: new Date(Date.now() - 120000).toLocaleString(), user: "advisor@firm.com", action: "Comparison generated", details: "Current vs 3 alternatives" },
  { timestamp: new Date(Date.now() - 240000).toLocaleString(), user: "advisor@firm.com", action: "Suitability analysis run", details: "Score: 85/100" },
  { timestamp: new Date(Date.now() - 360000).toLocaleString(), user: "advisor@firm.com", action: "Suitability data updated", details: "Client objectives modified" },
];
...
export function getMockComparisonData(alert: RenewalAlert): ComparisonData {
  return {
    current: {
      ...
      riders: ["Tax-Deferred Growth", "Free Annual Withdrawal (10%)", "Nursing Home Waiver"],
    },
    alternatives: [
      {
        ...
        mvaPenalty: "None",
        licensingApproved: true,
        licensingDetails: "Active carrier appointment confirmed. Licensed in 47 states.",
      ...
      },
      {
        ...
        licensingDetails: "Carrier appointment pending. Product training required.",
      ...
      },
      {
        ...
        mvaPenalty: "-2.5%",
        licensingApproved: true,
        licensingDetails: "Fully approved. Active carrier appointment current through 12/31/2026.",
        riders: ["2% Premium Bonus", "Tax-Deferred Growth", "Free Annual Withdrawal (10%)"],
      },
    ],
    missingData: false,
  };
