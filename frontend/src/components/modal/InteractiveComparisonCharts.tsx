...
export function InteractiveComparisonCharts({ products }: Props) {
  const [selected, setSelected] = useState<string[]>(products.slice(0, 4).map((p) => p.id));
  ...
  useEffect(() => {
    ...
    setSelected((prev) => {
      ...
      return filtered.length > 0 ? filtered : products.slice(0, 4).map((p) => p.id);
    });
  }, [products]);
  ...
  const getColor = (id: string) => COLORS[products.findIndex((p) => p.id === id) % COLORS.length];
  ...
  const selectedProducts = validSelected.map((id) => products.find((p) => p.id === id)).filter(Boolean) as typeof products;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border border-slate-200 rounded-xl p-5 bg-white">
        <h3 className="text-lg font-bold text-slate-900 mb-1">Interactive Product Comparison</h3>
        <p className="text-sm text-slate-600 mb-4">Select up to 4 products to compare</p>
          ...
          {products.map((p) => {
            ...
            return (
              <button key={p.id} onClick={() => toggle(p.id)}
                ...
                style={sel ? { borderColor: color, backgroundColor: `${color}15`, color } : {}}>
                <div className="flex items-center gap-2">
                  {sel && <div className="h-3 w-3 rounded-full" style={{ backgroundColor: color }} />}
                  <span>{p.name}</span>
      ...
      <div className="border border-slate-200 rounded-xl p-6 bg-white">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-slate-900">{chartMode === "value" ? "Account Value Projections" : "Interest Rate Projections"}</h3>
              <p className="text-sm text-slate-600">{chartMode === "value" ? "Projected account values over 20 years" : "Expected rate scenarios for the first 10 years"}</p>
            </div>
          </div>
        ...
        {chartMode === "value" ? (
          <ResponsiveContainer width="100%" height={380}>
            <ComposedChart data={perfData}>
              ...
              <RechartsTooltip
                contentStyle={{ backgroundColor: "white", border: "2px solid #cbd5e1", borderRadius: 8 }}
                formatter={(value: unknown, name?: string) => [`$${Number(value).toLocaleString()}`, (name ?? "").replace("_value", "")]}
              />
              <Legend formatter={(v: string) => v.replace("_value", "")} />
              {selectedProducts.map((p) => (
                <Line key={p.id} type="monotone" dataKey={`${p.name}_value`} name={`${p.name}_value`}
                  stroke={getColor(p.id)} strokeWidth={3} dot={false} activeDot={{ r: 6 }} />
              ))}
            </ComposedChart>
          </ResponsiveContainer>
        ) : (
          <ResponsiveContainer width="100%" height={380}>
            <AreaChart data={rateData}>
              ...
              <RechartsTooltip formatter={(v: unknown) => `${Number(v).toFixed(2)}%`}
                contentStyle={{ backgroundColor: "white", border: "2px solid #cbd5e1", borderRadius: 8 }} />
              <Legend formatter={(v: string) => v.replace("_expected", "")} />
              {selectedProducts.map((p) => (
                <Area key={p.id} type="monotone" dataKey={`${p.name}_expected`} name={`${p.name}_expected`}
                  stroke={getColor(p.id)} fill={getColor(p.id)} fillOpacity={0.15} strokeWidth={2} />
              ))}
            </AreaChart>
          </ResponsiveContainer>
        )}
      ...
      <div className="border border-slate-200 rounded-xl p-6 bg-white">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="font-bold text-slate-900">Guaranteed Annual Income Comparison</h3>
        </div>
        <p className="text-sm text-slate-600 mb-4">Projected annual income at key milestones (4.5% withdrawal rate)</p>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={incomeData} barCategoryGap="20%">
            ...
            <Legend />
            {selectedProducts.map((p) => (
              <Bar key={p.id} dataKey={p.name} name={p.name} fill={getColor(p.id)} radius={[4, 4, 0, 0]} />
            ))}
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="border border-slate-200 rounded-xl p-6 bg-gradient-to-br from-blue-50 to-indigo-50">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-bold text-slate-900 mb-2">Comparison Insights</h3>
            <div className="space-y-2 text-sm text-slate-700">
              <p>
                <span className="font-semibold">Rate Range:</span>{" "}
                {Math.min(...selectedProducts.map((p) => p.currentRate)).toFixed(2)}% –{" "}
                {Math.max(...selectedProducts.map((p) => p.currentRate)).toFixed(2)}% current rates
              </p>
              <p>
                <span className="font-semibold">Surrender Periods:</span>{" "}
                {selectedProducts.map((p) => `${p.name} (${p.surrenderYears} yrs)`).join(", ")}
              </p>
              <p>
                <span className="font-semibold">20-Year Projected Value:</span>{" "}
                {selectedProducts.map((p) => {
                  const last = p.performanceData[p.performanceData.length - 1];
