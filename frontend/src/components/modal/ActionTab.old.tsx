...
export function ActionTab({
  ...
  const allDisclosuresAcked = disclosureItems
    .filter((d) => d.required)
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-5">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <EditableField
              ...
              onChange={(v) => updateField("clientObjectives", v)}
            />
            <EditableField
              label="Risk Tolerance"
              value={suitabilityData.riskTolerance}
              onChange={(v) => updateField("riskTolerance", v)}
            />
            <EditableField
              label="Time Horizon"
              value={suitabilityData.timeHorizon}
              onChange={(v) => updateField("timeHorizon", v)}
            />
            <EditableField
              label="Liquidity Needs"
              value={suitabilityData.liquidityNeeds}
              onChange={(v) => updateField("liquidityNeeds", v)}
            />
            <EditableField
              label="Guaranteed Income"
              value={suitabilityData.guaranteedIncome}
              onChange={(v) => updateField("guaranteedIncome", v)}
            />
            <EditableField
              label="Advisor Eligibility"
              value={suitabilityData.advisorEligibility}
              onChange={(v) => updateField("advisorEligibility", v)}
            />
          </div>
        ...
        {!allDisclosuresAcked && (
          <div className="flex items-center gap-2 p-4 bg-amber-50 border border-amber-200 rounded-lg mb-4">
            ...
            <span className="text-sm text-amber-800">
              Complete all disclosures above to proceed with transaction.
            </span>
          </div>
        )}
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full text-sm px-2 py-1 border border-slate-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
      />
    </div>
  );
}
