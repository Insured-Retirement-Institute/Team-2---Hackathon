...
import { X, BellOff, Clock, AlertTriangle, CheckCircle2, ChevronDown, FileText } from "lucide-react";
  ...
  if (loading || !detail) {
    return (
      <Dialog open={open} onOpenChange={onClose}>
        <DialogContent className="!max-w-[98vw] !w-[98vw] !h-[96vh] p-0 flex items-center justify-center">
          <DialogTitle className="sr-only">Loading</DialogTitle>
          <DialogDescription className="sr-only">Loading alert details</DialogDescription>
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="!max-w-[98vw] !w-[98vw] !h-[96vh] p-0 gap-0 flex flex-col">
        <DialogTitle className="sr-only">
          Review for {alert.clientName} - Policy {alert.policyId}
        </DialogTitle>
        <DialogDescription className="sr-only">
          Review alert for policy {alert.policyId}. Current rate: {alert.currentRate}, Days until renewal: {alert.daysUntilRenewal}.
        </DialogDescription>
        ...
        <div className="h-16 px-8 border-t border-slate-200 flex items-center justify-between bg-gradient-to-r from-slate-50 to-slate-100 shrink-0">
          ...
          <Button onClick={onClose} className="bg-blue-600 hover:bg-blue-700 h-10 px-6 font-semibold">
            Close
          </Button>
        </div>
      </DialogContent>
