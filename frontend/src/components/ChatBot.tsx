...
const SUGGESTED_PROMPTS = [
  { icon: FileText, label: "Tell me about this product", prompt: "Can you explain the key features and benefits of the current policy?", category: "Product Info" },
  { icon: TrendingUp, label: "Compare renewal options", prompt: "What are the main differences between the current policy and the recommended renewal options?", category: "Analysis" },
  { icon: Shield, label: "Suitability considerations", prompt: "What factors should I consider for suitability when recommending this renewal?", category: "Compliance" },
  { icon: User, label: "Client communication tips", prompt: "How should I explain the rate change to my client in simple terms?", category: "Communication" },
];

const WELCOME: ChatMessage = {
  id: "welcome",
  role: "assistant",
  content: "Hello! I'm your AI Assistant for Fixed Annuity Renewals. I can help you understand products, analyze suitability, compare options, and answer compliance questions. How can I assist you today?",
                  ...
                  return (
                    <button key={i} onClick={() => send(p.prompt)}
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-slate-900 group-hover:text-blue-900">{p.label}</p>
          ...
          <div className="px-4 py-4 border-t border-slate-200 bg-white rounded-b-2xl">
            <div className="flex gap-2">
              <input ref={inputRef} type="text" value={input} onChange={(e) => setInput(e.target.value)}
                ...
                placeholder="Ask me anything..." className="flex-1 px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm" />
            ...
            <p className="text-xs text-slate-500 mt-2 px-1">AI responses are for guidance only. Verify all information before use.</p>
          </div>
        </div>
      )}
    </>
