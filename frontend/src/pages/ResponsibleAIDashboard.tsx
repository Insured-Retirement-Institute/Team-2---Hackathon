...
export function ResponsibleAIDashboard() {
  ...
  const loadStats = useCallback(async () => {
    try {
      setLoading(true);
      const data = await fetchResponsibleAIStats({ from_date: fromStr, to_date: toStr });
      setStats(data);
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-slate-900">Responsible AI Dashboard</h1>
                <p className="text-sm text-slate-600">Audit runs, explainability, and compliance</p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-8 py-8 space-y-8">
        ...
        <section>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="bg-white rounded-lg border border-slate-200 p-4">
                <p className="text-sm text-slate-600">Explainability coverage (Agent Two)</p>
                <p className="text-2xl font-bold text-slate-900">{stats.explainability_coverage_pct.toFixed(1)}%</p>
              </div>
              <div className="bg-white rounded-lg border border-slate-200 p-4">
                <p className="text-sm text-slate-600">Guardrail triggered</p>
                <p className="text-2xl font-bold text-slate-900">{stats.guardrail_triggered_count}</p>
              </div>
            </div>
          ) : (
            <p className="text-slate-600">No stats available. Ensure the API and database are configured.</p>
          )}
        </section>
        ...
        {stats && (
          <section>
            <h2 className="text-lg font-semibold text-slate-900 mb-4">By agent</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white rounded-lg border border-slate-200 p-4">
                <p className="font-medium text-slate-900">Agent One</p>
                <p className="text-sm text-slate-600">{stats.agent_one_runs} runs</p>
              </div>
              <div className="bg-white rounded-lg border border-slate-200 p-4">
                <p className="font-medium text-slate-900">Agent Two</p>
                <p className="text-sm text-slate-600">{stats.agent_two_runs} runs, {stats.agent_two_with_explanation} with explanation</p>
              </div>
              <div className="bg-white rounded-lg border border-slate-200 p-4">
                <p className="font-medium text-slate-900">Agent Three</p>
                <p className="text-sm text-slate-600">{stats.agent_three_runs} runs</p>
              </div>
            </div>
          </section>
        )}
            ...
            <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
              <Table>
                ...
                <TableBody>
                  {events.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={7} className="text-slate-600 text-center py-8">
                    ...
                    events.map((ev) => (
                      <TableRow key={ev.event_id}>
                        ...
                        <TableCell>{ev.input_validation_passed == null ? "—" : ev.input_validation_passed ? "Yes" : "No"}</TableCell>
                        <TableCell>{ev.guardrail_triggered == null ? "—" : ev.guardrail_triggered ? "Yes" : "No"}</TableCell>
                        <TableCell>
                          <Button variant="ghost" size="sm" onClick={() => openDetail(ev.event_id)}>
                            Detail
                          </Button>
                        </TableCell>
                      </TableRow>
      ...
      {/* Event detail modal */}
      <Dialog open={detailEvent !== null || detailLoading} onOpenChange={(open) => { if (!open) setDetailEvent(null); }}>
          ...
          {detailLoading ? (
            ...
            <div className="space-y-4 text-sm">
              <div className="grid grid-cols-2 gap-2">
                ...
                <span>{detailEvent.timestamp ? new Date(detailEvent.timestamp).toLocaleString() : "—"}</span>
                ...
                <span>{detailEvent.input_validation_passed == null ? "—" : detailEvent.input_validation_passed ? "Yes" : "No"}</span>
                {detailEvent.error_message && (
                  <>
                    <span className="text-slate-600">Error</span>
                    <span className="text-red-600">{detailEvent.error_message}</span>
                  </>
                )}
              </div>
              {detailEvent.explanation_summary && (
                <>
                  ...
                  <p className="text-slate-900">{detailEvent.explanation_summary}</p>
                </>
              )}
              {(detailEvent.data_sources_used?.length ?? 0) > 0 && (
                <>
                  <p className="text-slate-600 font-medium">Data sources used</p>
                  <ul className="list-disc pl-4 text-slate-900">{detailEvent.data_sources_used!.map((s, i) => <li key={i}>{s}</li>)}</ul>
                </>
              )}
              {(detailEvent.choice_criteria?.length ?? 0) > 0 && (
                <>
                  <p className="text-slate-600 font-medium">Choice criteria</p>
                  <ul className="list-disc pl-4 text-slate-900">{detailEvent.choice_criteria!.map((s, i) => <li key={i}>{s}</li>)}</ul>
                </>
              )}
              {detailEvent.input_summary && Object.keys(detailEvent.input_summary).length > 0 && (
                <>
                  <p className="text-slate-600 font-medium">Input summary</p>
                  <pre className="bg-slate-100 rounded p-2 text-xs overflow-x-auto">{JSON.stringify(detailEvent.input_summary, null, 2)}</pre>
                </>
              )}
              {detailEvent.payload && (
                <>
                  <p className="text-slate-600 font-medium">Full payload (Agent Two)</p>
                  <pre className="bg-slate-100 rounded p-2 text-xs overflow-x-auto max-h-60 overflow-y-auto">{JSON.stringify(detailEvent.payload.payload, null, 2)}</pre>
                </>
              )}
            </div>
          ) : null}
